# This code is mainly excerpted from code of RIQL.
# https://github.com/YangRui2015/RIQL/attack.py
...
MODEL_PATH = {
    "EDAC": str(current_path / "pretrained_model" / "EDAC") + "/EDAC_baseline_seed0-",  ### to be added
}
...
class Attack:
    ...
    def output_attack_indexs(self):
        if self.total_attack_indexs is not None:
            return np.array(self.total_attack_indexs)
        raise ValueError(self.total_attack_indexs)
    ...
    def set_attack_config(
        ...
        if self.attack_indexs is None or self.resample_indexs:
            ...
            if self.attack_times == 1:
                self.total_attack_indexs = list(self.attack_indexs)
            elif self.attack_times > 1:
                self.total_attack_indexs += list(self.attack_indexs)
                self.total_attack_indexs = list(set(self.total_attack_indexs))
            else:
                raise NotImplementedError(self.attack_times)
    ...
    def load_model(self):
        ...
        from algo.EDAC import Actor, VectorizedCritic
    ...
    def loss_Q_for_next_obs(self, para, observation, action, std):
        ...
        qvalue = self.critic(noised_obs, action[0])
        return qvalue.mean()
...
def attack_dataset(config, dataset, use_original=False):
    ...
    attack_agent = Attack(
        ...
        force_attack=False, 
        device=hydra.utils.instantiate(config.device),
        seed=config.seed,
    )
    ...
    return dataset, attack_agent.output_attack_indexs()
